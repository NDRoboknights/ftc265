...
/// Convert a TimePoint to std::time_t
        LLVM_ATTRIBUTE_ALWAYS_INLINE std::time_t
        toTimeT(TimePoint<>
        TP) {
...
/// Convert a std::time_t to a TimePoint
    ...
    toTimePoint(std::time_t
    T) {
    using namespace std::chrono;
    return

    time_point_cast<seconds> (system_clock::from_time_t(T));
}
...
time_point_cast<nanoseconds> (system_clock::from_time_t(T))
