...
namespace rs2 {
    ...
    public:
        ...
        */
        stream_profile clone(rs2_stream type, int index, rs2_format format) const {
            ...
            res._clone = std::shared_ptr<rs2_stream_profile>(ref, [](rs2_stream_profile *r) {
                rs2_delete_stream_profile(r);
            });
        ...
        */
        ...
        explicit stream_profile(const rs2_stream_profile *profile) : _profile(profile) {
            rs2_error *e = nullptr;
            rs2_get_stream_profile_data(_profile, &_type, &_format, &_index, &_uid, &_framerate,
                                        &e);
            error::handle(e);
    ...
    public:
        ...
        * \return stream_profile - return the cloned stream profile.
        */
        stream_profile clone(rs2_stream type, int index, rs2_format format, int width, int height,
                             const rs2_intrinsics &intr) const {
            rs2_error *e = nullptr;
            auto ref = rs2_clone_video_stream_profile(_profile, type, index, format, width, height,
                                                      &intr, &e);
            error::handle(e);
            stream_profile res(ref);
            res._clone = std::shared_ptr<rs2_stream_profile>(ref, [](rs2_stream_profile *r) {
    ...
    public:
        ...
        */
        video_frame(const frame &f)
            ...
            rs2_error *e = nullptr;
            if (!f ||
                (rs2_is_frame_extendable_to(f.get(), RS2_EXTENSION_VIDEO_FRAME, &e) == 0 && !e)) {
                reset();
            }
            error::handle(e);
        }
    ...
    public:
        ...
        */
        depth_frame(const frame &f)
            ...
            rs2_error *e = nullptr;
            if (!f ||
                (rs2_is_frame_extendable_to(f.get(), RS2_EXTENSION_DEPTH_FRAME, &e) == 0 && !e)) {
                reset();
            }
            error::handle(e);
        }
    ...
    public:
        ...
        */
        disparity_frame(const frame &f)
            ...
            rs2_error *e = nullptr;
            if (!f ||
                (rs2_is_frame_extendable_to(f.get(), RS2_EXTENSION_DISPARITY_FRAME, &e) == 0 &&
                 !e)) {
                reset();
            }
            error::handle(e);
    ...
    public:
        ...
        */
        motion_frame(const frame &f)
            ...
            rs2_error *e = nullptr;
            if (!f ||
                (rs2_is_frame_extendable_to(f.get(), RS2_EXTENSION_MOTION_FRAME, &e) == 0 && !e)) {
                reset();
            }
            error::handle(e);
        }
    ...
    public:
        ...
        */
        pose_frame(const frame &f)
            ...
            rs2_error *e = nullptr;
            if (!f ||
                (rs2_is_frame_extendable_to(f.get(), RS2_EXTENSION_POSE_FRAME, &e) == 0 && !e)) {
                reset();
            }
            error::handle(e);
        }
    ...
    public:
        ...
        */
        frameset(const frame &f)
            ...
            rs2_error *e = nullptr;
            if (!f ||
                (rs2_is_frame_extendable_to(f.get(), RS2_EXTENSION_COMPOSITE_FRAME, &e) == 0 &&
                 !e)) {
                reset();
        ...
        */
        frame first_or_default(rs2_stream s, rs2_format f = RS2_FORMAT_ANY) const {
            ...
            foreach_rs([&result, s, f](frame frm) {
                if (!result && frm.get_profile().stream_type() == s &&
                    (f == RS2_FORMAT_ANY || f == frm.get_profile().format())) {
                    result = std::move(frm);
                }
            });
            return result;
