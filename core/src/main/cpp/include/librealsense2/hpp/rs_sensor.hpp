...
namespace rs2 {
    ...
    public:
        explicit notifications_callback(T on_notification) : on_notification_function(
                on_notification) {}
        ...
        */
        ...
        void set_notifications_callback(T callback) const {
            ...
            rs2_set_notifications_callback_cpp(_sensor.get(),
                                               new notifications_callback<T>(std::move(callback)),
                                               &e);
            error::handle(e);
        }
    ...
    inline bool operator==(const sensor &lhs, const sensor &rhs) {
        ...
        return std::string(lhs.get_info(RS2_CAMERA_INFO_NAME)) == rhs.get_info(RS2_CAMERA_INFO_NAME)
               && std::string(lhs.get_info(RS2_CAMERA_INFO_SERIAL_NUMBER)) ==
                  rhs.get_info(RS2_CAMERA_INFO_SERIAL_NUMBER);
    }
    ...
    public:
        color_sensor(sensor s)
                : sensor(s.get()) {
            rs2_error *e = nullptr;
            if (rs2_is_sensor_extendable_to(_sensor.get(), RS2_EXTENSION_COLOR_SENSOR, &e) == 0 &&
                !e) {
                _sensor.reset();
            }
            error::handle(e);
    ...
    public:
        motion_sensor(sensor s)
                : sensor(s.get()) {
            rs2_error *e = nullptr;
            if (rs2_is_sensor_extendable_to(_sensor.get(), RS2_EXTENSION_MOTION_SENSOR, &e) == 0 &&
                !e) {
                _sensor.reset();
            }
            error::handle(e);
    ...
    public:
        fisheye_sensor(sensor s)
                : sensor(s.get()) {
            rs2_error *e = nullptr;
            if (rs2_is_sensor_extendable_to(_sensor.get(), RS2_EXTENSION_FISHEYE_SENSOR, &e) == 0 &&
                !e) {
                _sensor.reset();
            }
            error::handle(e);
    ...
    public:
        ...
        void set_region_of_interest(const region_of_interest &roi) {
            rs2_error *e = nullptr;
            rs2_set_region_of_interest(_sensor.get(), roi.min_x, roi.min_y, roi.max_x, roi.max_y,
                                       &e);
            error::handle(e);
        }
        ...
        region_of_interest get_region_of_interest() const {
            ...
            rs2_error *e = nullptr;
            rs2_get_region_of_interest(_sensor.get(), &roi.min_x, &roi.min_y, &roi.max_x,
                                       &roi.max_y, &e);
            error::handle(e);
            return roi;
        }
    ...
    public:
        depth_sensor(sensor s)
                : sensor(s.get()) {
            rs2_error *e = nullptr;
            if (rs2_is_sensor_extendable_to(_sensor.get(), RS2_EXTENSION_DEPTH_SENSOR, &e) == 0 &&
                !e) {
                _sensor.reset();
            }
            error::handle(e);
    ...
    public:
        depth_stereo_sensor(sensor s) : depth_sensor(s) {
            rs2_error *e = nullptr;
            if (_sensor &&
                rs2_is_sensor_extendable_to(_sensor.get(), RS2_EXTENSION_DEPTH_STEREO_SENSOR, &e) ==
                0 && !e) {
                _sensor.reset();
            }
            error::handle(e);
    ...
    public:
        pose_sensor(sensor s)
                : sensor(s.get()) {
            rs2_error *e = nullptr;
            if (rs2_is_sensor_extendable_to(_sensor.get(), RS2_EXTENSION_POSE_SENSOR, &e) == 0 &&
                !e) {
                _sensor.reset();
            }
            error::handle(e);
        ...
        bool import_localization_map(const std::vector<uint8_t> &lmap_buf) const {
            rs2_error *e = nullptr;
            auto res = rs2_import_localization_map(_sensor.get(), lmap_buf.data(),
                                                   uint32_t(lmap_buf.size()), &e);
            error::handle(e);
            return !!res;
        }
         ...
         * \return true if success.
         */
        bool set_static_node(const std::string &guid, const rs2_vector &pos,
                             const rs2_quaternion &orient) const {
            rs2_error *e = nullptr;
         ...
         */
        bool
        get_static_node(const std::string &guid, rs2_vector &pos, rs2_quaternion &orient) const {
            rs2_error *e = nullptr;
    ...
    public:
        wheel_odometer(sensor s)
                : sensor(s.get()) {
            rs2_error *e = nullptr;
            if (rs2_is_sensor_extendable_to(_sensor.get(), RS2_EXTENSION_WHEEL_ODOMETER, &e) == 0 &&
                !e) {
                _sensor.reset();
            }
            error::handle(e);
        ...
        */
        bool load_wheel_odometery_config(const std::vector<uint8_t> &odometry_config_buf) const {
            rs2_error *e = nullptr;
            auto res = rs2_load_wheel_odometry_config(_sensor.get(), odometry_config_buf.data(),
                                                      uint32_t(odometry_config_buf.size()), &e);
            error::handle(e);
            return !!res;
        }
        ...
        * \return true on success
        */
        bool send_wheel_odometry(uint8_t wo_sensor_id, uint32_t frame_num,
                                 const rs2_vector &translational_velocity) {
            rs2_error *e = nullptr;
            auto res = rs2_send_wheel_odometry(_sensor.get(), wo_sensor_id, frame_num,
                                               translational_velocity, &e);
            error::handle(e);
            return !!res;
        }
