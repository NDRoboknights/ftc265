...
import com.roboknights4348.lib.wpimath.src.main.java.edu.wpi.first.wpilibj.system.NumericalJacobian;
import com.roboknights4348.lib.wpimath.src.main.java.edu.wpi.first.wpilibj.system.RungeKutta;
...
public class UnscentedKalmanFilter<States extends Num, Inputs extends Num, Outputs extends Num>
    ...
    public void predict(Matrix<Inputs, N1> u, double dtSeconds) {
        ...
        Matrix<States, States> discQ = Discretization.discretizeAQTaylor(contA, m_contQ, dtSeconds).getSecond();

        Matrix sigmas = m_pts.sigmaPoints(m_xHat, m_P);
    ...
    public <R extends Num> void correct(
            ...
            BiFunction<Matrix<States, N1>, Matrix<States, N1>, Matrix<States, N1>> residualFuncX,
            BiFunction<Matrix<States, N1>, Matrix<States, N1>, Matrix<States, N1>> addFuncX) {
        final Matrix discR = Discretization.discretizeR(R, m_dtSeconds);

        // Transform sigma points into measurement space
        Matrix<R, ?> sigmasH = new Matrix<>(new SimpleMatrix(rows.getNum(), 2 * m_states.getNum() + 1));
        Matrix<States, ?> sigmas = m_pts.sigmaPoints(m_xHat, m_P);
        for (int i = 0; i < m_pts.getNumSigmas(); i++) {
